...
def cv_quiver_plot(analysisGUI, input_param, local_act_time, conduction_vel):
    ...
    tck = bisplrep(x_arrow, y_arrow, z_mag)
    grid_z_mag = bisplev(xi, yi, tck)
    ...
    conduction_vel.quiver_plot_axis.contour(xi, yi, grid_z_mag, extend='both')
    CS = conduction_vel.quiver_plot_axis.contourf(xi, yi, grid_z_mag, extend='both',
        cmap="jet")
    # conduction_vel.vector_cbar = conduction_vel.quiver_plot_axis.figure.colorbar(
    #     mappable, ax=conduction_vel.quiver_plot_axis)
    cbar = conduction_vel.quiver_plot.colorbar(CS)
